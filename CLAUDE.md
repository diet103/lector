...
- **P3 text entry points: code complete (2026-07-21), pending on-device verification.** `entry/ReadAloudActivity` (translucent, `excludeFromRecents`, no `screenOrientation`) handles `ACTION_PROCESS_TEXT` (selection-toolbar "Read aloud") + `ACTION_SEND` `text/plain` (share sheet). `IntentTextExtractor` (pure) flattens the CharSequence, surfaces the read-only flag, caps via `SentenceCap` (sentence-boundary, 5k default). Scrim stays up until the service reports `isPlaying` (keep-alive handoff), then `finish()`; 8 s timeout; truncation announced via Toast. **Contract: never calls `setResult()`** (a result replaces the caller's selection in editable fields — risk #4); enforced by the documented instrumented test, guarded by the code comment. 29 unit tests green (added `IntentTextExtractorTest` ×8, `SentenceCapTest` ×5). No-key path shows a "finish setup" message (P5 wires the real onboarding bounce).
  **VERIFY ON PIXEL (PLAN §6 P3 exit):** select text in Chrome → "Read aloud" in the toolbar → audio, **and Chrome's selection is undisturbed** (test the Gmail-compose editable case too). Share text from another app → Lector reads it.
- **Then P4:** screenshot OCR path (`image/*` SEND, ML Kit) per PLAN §6.
