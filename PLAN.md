...
- **Reading history (requested 2026-07-21, unscheduled — recommend after v0.1)**: "last thing you read" on Home is the small half and worth doing on its own; a full history screen is the larger half. The argument *for* is that the audio cache already exists, so replaying a past read is free — a history screen is really a UI onto a cache the user currently can't reach, and it can honestly mark each entry "free to replay" vs "would cost ~N characters" once the 50 MB LRU has evicted it. The argument *against* is that it reverses PLAN §4's explicit "selected text never persisted": a durable list of everything you've read is a sensitive record in an app whose pitch is "the screenshot never leaves your phone". **If built: opt-in, one-tap clear-all, excluded from backup like the credential file, and never store screenshot images — only recognized text.**
