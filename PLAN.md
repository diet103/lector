...
- **Reddit sign-in — ❌ DECLINED 2026-07-22 by the user, after being scoped. Do not re-propose.** Anonymous access is gone for good (three User-Agents × three endpoints, 403 every time; and WSL2's mirrored networking means this box already tested from the phone's own wifi IP), so OAuth was the only route. What killed it was the cost of that route rather than the code: registration approval sits outside our control and can fail quietly, and the free tier's **100 QPM is per client_id shared by every user of the app** — a real ceiling whose only escape is a commercial licence Lector cannot use. **The screenshot path remains the answer for Reddit**, and `SharedLink.isWalledOff` stays exactly as it is, because refusing a Reddit link up front costs zero characters.
- **Backlog (unscheduled, discussed and deliberately deferred)**: QS-tile live capture (MediaProjection + per-session consent dialog) · accessibility-service one-tap read (reverses a locked constraint; Play-defensible as a genuine accessibility tool; prototype only post-v0.1) · tee-to-file streaming engine (the agreed cleaner long-term fix: one OkHttp writer into a growing file — structurally one POST, free seeking, known duration; confined to the `TtsEngineFactory` seam) · queue mode · chunked playlist for over-cap texts · Play closed-testing track · F-Droid (NonFreeNet + proprietary-ML-Kit questions) · playback resumption.
